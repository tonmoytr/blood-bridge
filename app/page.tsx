  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-red-500 selection:text-white overflow-x-hidden">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-500 ${
        scrollY > 20 ? "bg-white/80 backdrop-blur-md shadow-sm py-4" : "bg-transparent py-6"
      }`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-2 group cursor-pointer">
            ...
            <span className={`text-2xl font-bold tracking-tight transition-colors ${
              scrollY > 20 ? "text-slate-900" : "text-white"
            }`}>
              BloodBridge
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Mission', 'Donors', 'Requests', 'Community'].map((item) => (
              <a 
                key={item} 
                href={`#${item.toLowerCase()}`} 
                className={`text-sm font-medium transition-colors hover:text-red-500 ${
                  scrollY > 20 ? "text-slate-600" : "text-slate-200"
                }`}
              >
                {item}
              </a>
            ))}
            <Button className="bg-white text-red-600 hover:bg-red-50 border-none shadow-lg font-semibold rounded-full px-6">
              Download App
            </Button>
          </div>
        </div>
      ...
      {/* --- HERO SECTION --- */}
      {/* Changed min-h to min-h-screen for better viewport fit, kept z-20 */}
      <section className="relative min-h-[105vh] flex items-center justify-center overflow-hidden bg-slate-950 text-white rounded-b-[4rem] shadow-2xl z-20">
        
        {/* Background Fluid Effects */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-[20%] -left-[10%] w-[60vw] h-[60vw] bg-red-600/20 rounded-full blur-[100px] animate-pulse mix-blend-screen"></div>
          <div className="absolute top-[20%] -right-[10%] w-[50vw] h-[50vw] bg-rose-600/10 rounded-full blur-[120px] mix-blend-screen"></div>
          
          {/* Interactive Cells */}
          <div 
            className="absolute w-32 h-32 bg-gradient-to-br from-red-500 to-red-700 rounded-full blur-xl opacity-40 transition-transform duration-100 ease-out"
            style={{ 
          ...
          />
          <div 
            className="absolute w-24 h-24 bg-red-600 rounded-full blur-md opacity-30 animate-float"
            style={{ top: '60%', left: '15%' }}
          />
          <div 
            className="absolute w-40 h-40 bg-red-800 rounded-full blur-2xl opacity-30 animate-float-delayed"
        ...
        <div className="container mx-auto px-6 relative z-10 pt-20 pb-32">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm mb-4 animate-fade-in-up">
              <span className="relative flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span>
              </span>
              <span className="text-sm font-medium text-red-200">Urgent: O- Blood needed in Dhaka</span>
            </div>

            <h1 className="text-6xl md:text-8xl font-bold tracking-tighter leading-[0.9]">
              <span className="block bg-gradient-to-b from-white to-white/70 bg-clip-text text-transparent">Bridging Hope</span>
              <span className="block text-red-500 italic relative">
                to Life
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
              ...
              <Button variant="outline" className="h-14 px-8 rounded-full border-slate-700 bg-transparent text-white hover:bg-white/10 text-lg font-medium hover:border-white transition-all">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-12 opacity-80">
              {[
                ...
                <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-sm hover:bg-white/10 transition-colors">
                  ...
                  <div className="text-xs text-slate-400 uppercase tracking-wider">{stat.l}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 animate-bounce text-white/30">
          <ArrowRight className="rotate-90" />
        </div>
      </section>

      {/* --- LIVE ACTIVITY TICKER (FIXED) --- */}
      {/* 
         FIX: Changed z-10 to z-30 so it floats ON TOP of the hero section. 
         Changed -mt-8 to -mt-12 so it pulls up slightly more to bridge the rounded corner gap.
      */}
      ...
      <section id="how-it-works" className="py-32 bg-slate-950 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-20">
            <h2 className="text-4xl font-bold mb-4">How We Save Lives</h2>
            <p className="text-slate-400">Three simple steps to becoming a hero in someone's story.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={idx} className="group relative p-1 rounded-2xl bg-gradient-to-b from-red-500/20 to-transparent hover:from-red-500/50 transition-all duration-500">
                ...
                <div className="relative h-full bg-slate-900 p-8 rounded-xl border border-slate-800 overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-10 text-9xl font-bold font-serif text-white select-none">
                    {item.step}
                  </div>
      ...
      <section className="relative py-24 bg-white overflow-hidden">
        <div className="container mx-auto px-6">
          ...
          <footer className="mt-20 border-t border-slate-200 pt-10 flex flex-col md:flex-row justify-between items-center gap-6 text-slate-500 text-sm">
